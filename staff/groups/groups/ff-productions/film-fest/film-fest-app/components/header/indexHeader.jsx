...
return <header id="top">
            <nav className="main-bar">
                ...
                <p key={Math.random()} className="main-bar__genre header-button" onClick={event=>{
                    event.preventDefault()
                    onGoGenre()
                }}>GENRE</p>

                <ul key={Math.random()}>{onGenres && onGenres.map((genre, index) => <GenresItem onKey={index} onGenre={genre} getMoviesByGenre={onGetMoviesByGenre}/>) }</ul>
